(function(){
'use strict';
var sb=null,session=null,profile=null,observer=null,syncing=false;
...
function syncHome(){if(syncing)return;syncing=true;try{inject();labelLiveSection();document.documentElement.classList.remove('angel-home-switching')}finally{syncing=false}}
function relevantMutation(ms){for(var i=0;i<ms.length;i++){var ns=ms[i].addedNodes||[];for(var j=0;j<ns.length;j++){var n=ns[j];if(n.nodeType!==1)continue;if((n.matches&&n.matches('main.wrap,.hero,.filters,#app'))||(n.querySelector&&n.querySelector('main.wrap,.hero,.filters')))return true}}return false}
...
function bind(){document.addEventListener('click',function(e){var home=e.target.closest&&e.target.closest('nav [data-id="home"]');if(home){document.documentElement.classList.add('angel-home-switching');queueMicrotask(syncHome);return}var b=e.target.closest&&e.target.closest('[data-portal-go]');if(!b)return;e.preventDefault();go(b.getAttribute('data-portal-go'))},true)}
async function boot(){sb=shared();if(sb){var s=await sb.auth.getSession();session=s.data.session;if(session)await loadProfile();sb.auth.onAuthStateChange(async function(_e,ses){session=ses;if(session)await loadProfile();else profile=null;var old=q('#angelPortal');if(old)old.remove();syncHome()})}bind();syncHome();observer=new MutationObserver(function(ms){if(relevantMutation(ms))syncHome()});observer.observe(document.body,{childList:true,subtree:true})}
